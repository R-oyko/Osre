...
class Effect:
    ...
    def __init__(self, pos, color, kind):
        self.pos = pos
        self.color = color
        self.kind = kind  # 'hit' -> expanding ring, 'miss' -> falling X
    ...
    def draw(self, surface):
        t = self.progress
        alpha = int(255 * (1 - t))  # fades out as it plays

        if self.kind == 'hit':
            # Expanding, fading ring in the object's combo color.
            ...
            # A red "X" that drifts upward and fades out.
