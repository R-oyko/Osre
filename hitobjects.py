...
# What step()/try_hit() return once an object is fully judged: whether it
# counted as a hit, where on screen it happened (for the effect pop), and
# which combo color to spawn that effect in.
...
class HitObject:
    ...
    def __init__(self, x, y, time, number, color, new_combo=False):
        ...
        self.stack_index = 0  # set later by compute_stacking()
...
class Slider(HitObject):
    ...
    def __init__(self, x, y, time, number, color, new_combo, *, end_time, points,
        ...
        self.body_cache = None      # pre-rendered body surface, built on first draw
        self.head_hit = False       # whether the head circle has been clicked
        self.holding = False        # whether the player is currently tracking the ball
        self.ok = False             # whether they're *currently* within HOLD_TOLERANCE
        self.held_time = 0.0        # ms of the slider's duration actually tracked so far
        self.last_ms = time         # elapsed time as of the last held_time update
    ...
    def position_at(self, t):
        ...
        # Binary search for the path segment containing target_dist, so the
        # ball moves at a genuinely constant speed along the real curve
        # instead of the straight-line skeleton between raw control points.
        lo, hi = 0, len(cum) - 1
        while lo < hi:
            mid = (lo + hi) // 2
    ...
    def try_hit(self, mouse_pos, elapsed):
        ...
        if self.head_hit:
            return None  # already hit -- ignore extra clicks so the sound can't be spammed
        pos = self.screen_pos
        if math.dist(mouse_pos, pos) <= CIRCLE_RADIUS and abs(elapsed - self.time) <= HIT_WINDOW:
            self.head_hit = True
            self.holding = True
    ...
    def step(self, surface, elapsed, mouse_pos, holding_input):
        ...
        # Body: rendered once and cached, then just blitted every frame.
        ...
        # Reverse arrow, if a repeat is coming up at either end.
        ...
        # Approach ring, before the slider actually starts.
        time_until_hit = self.time - elapsed
        if time_until_hit > -HIT_WINDOW:
        ...
        # Ball + hold tracking, once the slider has actually started.
        if elapsed >= self.time:
            raw_x, raw_y = to_screen_pos(*self.position_at(elapsed))
            ball_pos = (raw_x + stack_dx, raw_y + stack_dy)

            # "Leash" ring showing how far you can drift and still count.
            ...
            # Same translucent treatment as every other hit circle (combo
            # color + CIRCLE_FILL_ALPHA via draw_hit_circle) instead of a
            # fully opaque white/black ball, so the slider ball no longer
            # stands out against its own (translucent) body.
            draw_hit_circle(surface, ball_pos, CIRCLE_RADIUS, self.color)

        # Final judgement, once the slider's time is up.
...
class Spinner(HitObject):
    ...
    def __init__(self, x, y, time, number, color, new_combo, end_time):
        ...
        self.end_time = end_time
        self.rotations = 0.0
        self.last_angle = None  # angle of mouse relative to center, last frame
    ...
    def step(self, surface, elapsed, mouse_pos, holding_input):
        ...
        if elapsed >= self.time and holding_input:
            ...
            self.last_angle = None  # input released -- next frame starts a fresh angle delta
...
def build_hit_objects(raw_objects):
    ...
    for i, raw in enumerate(raw_objects):
        ...
        if raw['type'] == 'circle':
            ...
            continue  # unknown type -- skip rather than crash on an odd beatmap
